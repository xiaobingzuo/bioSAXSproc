function varargout = bioSAXSmatV4(varargin)
% BIOSAXSMATV4 MATLAB code for bioSAXSmatV4.fig
%      BIOSAXSMATV4, by itself, creates a new BIOSAXSMATV4 or raises the existing
%      singleton*.
%
%      H = BIOSAXSMATV4 returns the handle to a new BIOSAXSMATV4 or the handle to
%      the existing singleton*.
%
%      BIOSAXSMATV4('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in BIOSAXSMATV4.M with the given input arguments.
%
%      BIOSAXSMATV4('Property','Value',...) creates a new BIOSAXSMATV4 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before bioSAXSmatV4_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to bioSAXSmatV4_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help bioSAXSmatV4

% Last Modified by GUIDE v2.5 29-Nov-2022 15:25:21

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @bioSAXSmatV2b_OpeningFcn, ...
                   'gui_OutputFcn',  @bioSAXSmatV2b_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before bioSAXSmatV4 is made visible.
function bioSAXSmatV2b_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to bioSAXSmatV4 (see VARARGIN)

% Choose default command line output for bioSAXSmatV4
...
% UIWAIT makes bioSAXSmatV4 wait for user response (see UIRESUME)
...
bioSAXSData.avg.reportDoc = 'myReport.doc';
bioSAXSData.avg.resultDir = 'Processed';
...
function varargout = bioSAXSmatV2b_OutputFcn(hObject, eventdata, handles) 
...
if strcmp(bioSAXSData.avg.resultDir, 'Processed')
    avgDir = fullfile(bioSAXSData.avg.dir(1:end-13), 'Processed');  %Nov 19, 2022
else
    avgDir = bioSAXSData.avg.resultDir;
end
...
%%%%%%%% doc %%%%%%%%
docFileSpec = fullfile(avgDir,bioSAXSData.avg.reportDoc);
[ActXWord,WordHandle]=StartWord(docFileSpec);
ActXWord.Selection.Pagesetup.TopMargin=26; % 1cm = 28.34646 points
ActXWord.Selection.Pagesetup.BottomMargin=24;

fprintf('Averaging and processing reports will be saved in %s\n',docFileSpec);
%%create header in word        
Style='Heading 1'; %NOTE! if you are NOT using an English version of MSWord you get
 
TextString='SAXS Data Processing at 12-ID-B of APS';
WordText(ActXWord,TextString,Style,[0,2]);%two enters after text

Style='Normal';
TextString = sprintf('1-D data sets directory: %s.', bioSAXSData.avg.dir);    
WordText(ActXWord,TextString,Style,[0,1]);%enter after text
TextString = sprintf('Time: %s.\n\n', datetime('now'));    
WordText(ActXWord,TextString,Style,[0,1]);%enter after text
TextString = sprintf('### Outerlier detection parameters: %s. ###\n', num2str(outlierParam));    
WordText(ActXWord,TextString,Style,[0,1]);%enter after text
...
for kk=1:length(bioSAXSData.avg.fileSer)
    ...
    if length(fns)>=3
        ...
        %%%%%%%%%%%%%%%%%%doc%%%%%%%%%%%%%%%%%%%%%%%%
        Style='Heading 1'; 
        TextString= sprintf('File name with serial #: %s.\n', newAvgFn(1:end-4));
        WordText(ActXWord,TextString,Style,[0,1]);%one enter after text
        %%%%%%%%%%%%%%%%%%doc%%%%%%%%%%%%%%%%%%%%%%%%
...
%         outIndex = findCurveOutliers(iqs(q1pt:q2pt, :), bioSAXSData.avg.algorithm, outlierParam, 1);        
        ...
        if fid ~= -1
            ...
            %%%%%%%%%%%%%%%%%%doc%%%%%%%%%%%%%%%%%%%%%%%%
            Style='Normal';
            WordText(ActXWord,'All traces plot, outliers in RED.',Style,[0,1]);  
            FigureIntoWord(ActXWord); % add figure
            if sum(oIndex)>0
                WordText(ActXWord,'All good traces plot, average in BLUE.',Style,[1,1]);
                figure(1002); 
                loglog(q,iqs(:,oIndex)); xlabel('Q /A'); ylabel('Intensity'); txtStr=sprintf('%s,   good traces', newFn);  title(strrep(txtStr, '_', '\_')); hold on;
                loglog(q,datAvg, '-b', 'Linewidth',2); hold off; 
                saveas(gcf, fullfile(avgDir, [newFn '_good.jpg']));
                FigureIntoWord(ActXWord); % add figure
            else
                WordText(ActXWord,'Too many outliers! Measurements need to redo!',Style,[1,1]);
            end
            TextString = sprintf('Total %d scans in this serial: %s.\n', length(fns), num2str(scanNum));    
            WordText(ActXWord,TextString,Style,[1,0]);
            TextString = sprintf('Outliers scan #: %s.\n', num2str(scanNum(~oIndex)));    
            WordText(ActXWord,TextString,Style,[0,0]);
            %TextString = sprintf('Averaging starts at: %s; ends at: %s; takes %s.\n', t1, t2, t2-t1);   
            %WordText(ActXWord,TextString,Style,[0,0]);      
            ...
            if chkDamage
                ...
                if abs(damg) >= 5
                    ...
                    damStr=sprintf('%sSEVERE damage (>=5S). ', damStr);
                elseif abs(damg) >= 3
                    %fprintf(fid, 'SIGNIFICANT dammage(2-4S). \n');
                    damStr=sprintf('%sSIGNIFICANT damage(3-5S). ', damStr);
                elseif abs(damg) >= 1
                    %fprintf(fid, 'DETECTABLE dammage (1-2S). \n');
                    damStr=sprintf('%sDETECTABLE damage (1-3S). ', damStr);
                else
                    disp('NO damage in cleaned data!')
                end
                fprintf(fid, '%s\n', damStr);
                
                WordText(ActXWord,damStr(3:end),Style,[0,1]);
    ...
    if ~isempty(datAvg) && newFn(1)=='S'
        if contains(lower(newAvgFn), 'buf') || contains(lower(newAvgFn), 'bkg')
            ...
            WordText(ActXWord,'This is a BUFFER data set.',Style,[0,1]); 
        else
            bioSAXSData.avg.samp = [q datAvg datErr];
            bioSAXSData.avg.sampName = fullnewAvgFn;
            fprintf(fid2, '  This is a Sample data.\n');
            TextString = sprintf('This is a SAMPLE data set; Background Subtraction Figure is %s  as below:',[newFn '_bsub.jpg']);
            ...
            if ~isempty(bioSAXSData.avg.buf)
                ...
                bsubStr2 = sprintf('Sample file: %s\nBuffer file: %s\nalpha = 1.0;    Date & Time: %s', bioSAXSData.avg.sampName, bioSAXSData.avg.bufName, datetime);
                
                WordText(ActXWord, bsubStr2,Style,[0,1]);
                
                dlmwrite(bsubFn, bsubStr, 'delimiter', '');
                dlmwrite(bsubFn, bsub, 'delimiter', ' ', 'precision', '%.5E', '-append');

                figure(1003); loglog(Qbsub, iQbsub, '-r', Qbsub, bioSAXSData.avg.buf(:,2), '-b', Qbsub, bioSAXSData.avg.samp(:,2), '-g'); 
                ...
                %WordText(ActXWord, 'Background Subtraction Figure:',Style,[0,1]);
                FigureIntoWord(ActXWord); % add figure
                % analysis goes here
                ...
                try
                    ...
                    if ~isempty(result)
                        fprintf(fid2,'  Rg =  %6.2f  +/-  %6.2f;  Io =  %.3E  +/-  %.2E \n', result.Rg, result.RgErr, result.Io, result.IoErr);
                        fprintf(fid2,'  Quality : %3d%%; Aggregated: %4.2f\n\n\n', round(result.Qfit*100), result.aggr);
                        
                        WordText(ActXWord, 'Guinier Analysis Figure:',Style,[0,1]);
                        FigureIntoWord(ActXWord); % add figure
                        TextString = sprintf('Rg =  %6.3f  +/-  %6.3f A;  Io =  %.3E  +/-  %.2E', result.Rg, result.RgErr, result.Io, result.IoErr);
                        WordText(ActXWord, TextString ,Style,[0,1]);
                        TextString = sprintf('Fitting quality : %3d%%;    Aggregated (if < 0) : %4.2f', round(result.Qfit*100), result.aggr);
                        WordText(ActXWord, TextString ,Style,[0,1]);
                    end
                catch
...
WordCreateTOC(ActXWord,1,3);
CloseWord(ActXWord,WordHandle,docFileSpec);    
...
function reportDoc_Callback(hObject, eventdata, handles)
% hObject    handle to reportDoc (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of reportDoc as text
%        str2double(get(hObject,'String')) returns contents of reportDoc as a double
bioSAXSData = getData;
doc = 'myReport.doc';
str= get(handles.reportDoc, 'String');   
s = strsplit(str, '.');
if ~isempty(s)
    doc = sprintf('%s.doc',s{1});
end
bioSAXSData.avg.reportDoc = doc;

setData(bioSAXSData);

% --- Executes during object creation, after setting all properties.
function reportDoc_CreateFcn(hObject, eventdata, handles)
% hObject    handle to reportDoc (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% -------------------------------------------------------------------
% Below functions are adopted from File: WriteToWordFromMatlab
...
% --- Executes when uipanel40 is resized.
function uipanel40_SizeChangedFcn(hObject, eventdata, handles)
...
function avgResultDir_Callback(hObject, eventdata, handles)
% hObject    handle to avgResultDir (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of avgResultDir as text
%        str2double(get(hObject,'String')) returns contents of avgResultDir as a double
bioSAXSData = getData;
doc = 'Processed';
str= get(handles.avgResultDir, 'String');   

if ~isempty(str)
    doc = str;
end
bioSAXSData.avg.resultDir = doc;

setData(bioSAXSData);

% --- Executes during object creation, after setting all properties.
function avgResultDir_CreateFcn(hObject, eventdata, handles)
% hObject    handle to avgResultDir (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in selectResultDir.
function selectResultDir_Callback(hObject, eventdata, handles)
% hObject    handle to selectResultDir (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
bioSAXSData = getData;
bioSAXSData.avg.resultDir=uigetdir;
set(handles.avgResultDir, 'String', bioSAXSData.avg.resultDir);
setData(bioSAXSData);
