...
function varargout = bioSAXSprocess2(varargin)
% BIOSAXSPROCESS2 MATLAB code for bioSAXSprocess2.fig
%      BIOSAXSPROCESS2, by itself, creates a new BIOSAXSPROCESS2 or raises the existing
%      singleton*.
%
%      H = BIOSAXSPROCESS2 returns the handle to a new BIOSAXSPROCESS2 or the handle to
%      the existing singleton*.
%
%      BIOSAXSPROCESS2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in BIOSAXSPROCESS2.M with the given input arguments.
%
%      BIOSAXSPROCESS2('Property','Value',...) creates a new BIOSAXSPROCESS2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before bioSAXSprocess2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to bioSAXSprocess2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help bioSAXSprocess2

% Last Modified by GUIDE v2.5 16-Feb-2022 09:22:06

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @bioSAXSprocess2_OpeningFcn, ...
                   'gui_OutputFcn',  @bioSAXSprocess2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before bioSAXSprocess2 is made visible.
function bioSAXSprocess2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to bioSAXSprocess2 (see VARARGIN)

% Choose default command line output for bioSAXSprocess2
...
% UIWAIT makes bioSAXSprocess2 wait for user response (see UIRESUME)
...
%Guinier
bioSAXSData.atsasPath = 'C:\atsas\bin';
bioSAXSData.Rg = 1.0;
...
function varargout = bioSAXSprocess2_OutputFcn(hObject, eventdata, handles) 
...
if isfield(bioSAXSData,'mZeroConcFn') ~= 0 && isempty(bioSAXSData.mZeroConcFn) == 0
    ...
    Rgcmd= sprintf('%s "%s" -f ssv > "%s"', fullfile(bioSAXSData.atsasPath, 'autorg'), datFull, tmpLog); 
    dos(Rgcmd);
    fid=fopen(tmpLog,'r');
    line = fgetl(fid);
    fclose(fid);
...
if bioSAXSData.samp5Sel==1 && ~isempty(bioSAXSData.samp5) && ~isempty(bioSAXSData.buf5)
    ...
    std_samp=[std_samp bioSAXSData.samp5(:,3)]; 
...
gnomCMD = sprintf('%s %s -r %f -o %s > %s', fullfile(bioSAXSData.atsasPath, 'datgnom4'),saxsFile2, bioSAXSData.Rg, pddfFn, pddfLog);
dos(gnomCMD);
%dos(eval(gnomCMD));
...
% --- Executes on button press in selectMultiFiles.
function selectMultiFiles_Callback(hObject, eventdata, handles)
% hObject    handle to selectMultiFiles (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
bioSAXSData = getData;
...
% --- Executes on button press in selectATSAS.
function selectATSAS_Callback(hObject, eventdata, handles)
% hObject    handle to selectATSAS (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
bioSAXSData = getData;

atsasPath = uigetdir('C:\');
if atsasPath
    fprintf('Selected ATSAS/bin Path: %s\n', atsasPath);
    bioSAXSData.atsasPath = atsasPath;
    set(handles.atsasPath,'String', sprintf('Path: %s', bioSAXSData.atsasPath));
else
    fprintf('No ATSAS/bin Path Selected, Default path is: %s\n', bioSAXSData.atsasPath);
end
