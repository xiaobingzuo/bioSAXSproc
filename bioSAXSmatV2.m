function varargout = bioSAXSmatV2(varargin)
% BIOSAXSMATV2 MATLAB code for bioSAXSmatV2.fig
%      BIOSAXSMATV2, by itself, creates a new BIOSAXSMATV2 or raises the existing
%      singleton*.
%
%      H = BIOSAXSMATV2 returns the handle to a new BIOSAXSMATV2 or the handle to
%      the existing singleton*.
%
%      BIOSAXSMATV2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in BIOSAXSMATV2.M with the given input arguments.
%
%      BIOSAXSMATV2('Property','Value',...) creates a new BIOSAXSMATV2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before bioSAXSmatV2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to bioSAXSmatV2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help bioSAXSmatV2

% Last Modified by GUIDE v2.5 28-Nov-2022 22:26:28
...
% --- Executes just before bioSAXSmatV2 is made visible.
function bioSAXSmatV2b_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to bioSAXSmatV2 (see VARARGIN)

% Choose default command line output for bioSAXSmatV2
...
% UIWAIT makes bioSAXSmatV2 wait for user response (see UIRESUME)
...
%         outIndex = findCurveOutliers(iqs(q1pt:q2pt, :), bioSAXSData.avg.algorithm, outlierParam, 1);        
        ...
        if fid ~= -1
            ...
            %%%%%%%%%%%%%%%%%%doc%%%%%%%%%%%%%%%%%%%%%%%%
            Style='Normal';
            WordText(ActXWord,'All traces',Style,[0,1]);  
            ...
            if chkDamage
                ...
                WordText(ActXWord,damStr,Style,[0,1]);
    ...
    if ~isempty(datAvg) && newFn(1)=='S'
        if contains(lower(newAvgFn), 'buf') || contains(lower(newAvgFn), 'bkg')
            ...
            if ~isempty(bioSAXSData.avg.buf)
                ...
                WordText(ActXWord, bsubStr,Style,[0,1]);
                ...
                WordText(ActXWord, 'Background Subtraction',Style,[0,1]);
                FigureIntoWord(ActXWord); % add figure
                % analysis goes here
                ...
                try
                    ...
                    if ~isempty(result)
                        ...
                        WordText(ActXWord, 'Guinier Analysis',Style,[0,1]);
                        FigureIntoWord(ActXWord); % add figure
                        TextString = sprintf('  Rg =  %6.2f  +/-  %6.2f;  Rg =  %.3E  +/-  %.2E \n', result.Rg, result.RgErr, result.Io, result.IoErr);
                        WordText(ActXWord, TextString ,Style,[0,1]);
                        TextString = sprintf('  Quality : %3d%%; Aggregated: %4.2f\n\n\n', round(result.Qfit*100), result.aggr);
                        WordText(ActXWord, TextString ,Style,[0,1]);
                    end
                catch
